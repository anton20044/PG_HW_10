...
```
* Сгенерировал в нее данные
```
insert into test select generate_series, generate_series * 20 from generate_series(1,50000);
```
* Выставил параметры для логирования блокировок длительность более 200мс
```
alter system set log_lock_waits = on;
alter system set log_min_duration_statement = '200ms';
```
* Перезапустил СУБД
* В трех терминалах открыл транзакцию и запустил запрос на обновление одной и той же строки
```
begin;

update test set num = 9999 where id = 10;
```
* Посмотрел возникшие блокировки
```
select * from pg_locks where relation = 'test'::regclass;
```

![locks](image/dz08_01.jpg)

* Возникли блокировки:
 * RowExclusiveLock для таблицы, блокировка накладывается любой транзакцией, которая изменяет данные. В представлении три блокировки (по одной на каждую из транзакций и все блокировки предоставлены)
 * ExclusiveLock для строки, блокировка накладывается на строку, которую хотим поменять, доступ на строку будет разреше только на чтение. В представлении две блокировки (одна предоставлена, вторая нет), третья транзакция ожидает в очереди на запрос такой блокировки
* Воспроизвел блокировку трех транзакций друг другом. Для этого отрыл в трех терминалах транзакции, в первом терминале обновил строку с id=1, во втором с id=2, в третьем с id=3. Затем в первом терминале обновил строку с id=2, во втором с id=3, в третьем с id=1.
* СУБД принудительно отклонила одну транзакцию, для прекращения дедлока
* По логу можно определить что транзакции, открытые процессами 14737, 14732, 14848 спровоцировали дедлок

![locks1](image/dz08_02.jpg)

* В двух сеансах запустил обновление таблицы без where. Обновление всех строк таблицы было выполнено.
